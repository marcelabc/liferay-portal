 ...
 */
@Component(
	immediate = true, property = "de.data.definition.field.type=key_value",
	service = DEFieldType.class
)
public class DEKeyValueFieldType implements DEFieldType {
	...
	public DEDataDefinitionField deserialize(JSONObject jsonObject)
		...
		if (jsonObject.has("autoFocus")) {
			deDataDefinitionField.setCustomProperty(
				"autoFocus", jsonObject.getBoolean("autoFocus"));
		}
	...
	public void includeContext(
		...
		if (deDataDefinitionField.hasCustomProperty("autoFocus")) {
			context.put(
				"autoFocus",
				GetterUtil.getBoolean(
					deDataDefinitionField.getCustomProperty("autoFocus")));
		}
