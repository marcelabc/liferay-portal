...
public class ReportDefinitionFinderImpl
	extends ReportDefinitionFinderBaseImpl implements ReportDefinitionFinder {

	public static final String COUNT_BY_C_G_N_D =
		ReportDefinitionFinder.class.getName() + ".countByC_G_N_D";

	public static final String FIND_BY_C_G_N_D =
		ReportDefinitionFinder.class.getName() + ".findByC_G_N_D";
	...
	@Override
	public List<ReportDefinition> filterFindByKeywords(
	...
	@Override
	public List<ReportDefinition> filterFindByC_G(
	...
	@Override
	public List<ReportDefinition> filterFindByC_G_N_D(
		long companyId, long groupId, String[] names, String[] descriptions,
		...
		try {
			...
			if (inlineSQLHelper) {
				sql = InlineSQLHelperUtil.replacePermissionCheck(
					sql, ReportDefinition.class.getName(),
					"ReportDefinition.reportDefinitionId", groupId);
			}

			if (groupId <= 0) {
				sql = StringUtil.replace(
					sql, "(ReportDefinition.groupId = ?) AND",
					StringPool.BLANK);
			}

			sql = _customSQL.replaceKeywords(
				sql, "LOWER(ReportDefinition.name)", StringPool.LIKE, false,
				names);
			sql = _customSQL.replaceKeywords(
				sql, "ReportDefinition.description", StringPool.LIKE, true,
				descriptions);
	...
	protected List<ReportDefinition> doFindByC_G_N_D(
		long companyId, long groupId, String[] names, String[] descriptions,
		boolean andOperator, int start, int end,
		OrderByComparator<ReportDefinition> orderByComparator,
		boolean inlineSQLHelper) {
		...
		try {
			...
			q.addEntity("ReportDefinition", ReportDefinitionImpl.class);
