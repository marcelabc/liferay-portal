...
public class UpgradeDDMStructure extends UpgradeProcess {
	...
	protected void addDDMStructure(
			ResultSet resultSet, long newDDMStructureId, long newParentStructureId)
		throws Exception {
		...
		try (PreparedStatement ps = connection.prepareStatement(sql)) {
			...
			addDDMStructureLink(resultSet.getLong("companyId"), newDDMStructureId);
		}
	}
		...
		try (PreparedStatement ps1 = connection.prepareStatement(sql)) {
			...
			try (ResultSet rs = ps1.executeQuery()) {
				while (rs.next()) {
					...
					addDDMStructure(rs, newDDMStructureId, 0);
