 ...
 */
@Component(
	immediate = true, property = "de.data.definition.field.type=radio",
	service = DEFieldType.class
)
public class DERadioFieldType implements DEFieldType {
	...
	public void includeContext(
		...
		if (deDataDefinitionField.hasCustomProperty("predefinedValue")) {
			...
			context.put(
				"predefinedValue",
				DEDataEngineUtil.getValue(predefinedValue, jsonFactory));
		}

		if (deDataDefinitionField.hasCustomProperty("value")) {
			String value = DEDataEngineUtil.getValue(
